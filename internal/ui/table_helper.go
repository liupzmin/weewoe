...
func nsFilter(ns string, data render.TableData) render.TableData {
	ns = strings.TrimSpace(ns)

	if ns == client.NamespaceAll || ns == client.AllNamespaces {
		return data
	}
	...
	for _, s := range data.RowEvents {
		if s.Row.Fields[0] == ns {
			filtered.RowEvents = append(filtered.RowEvents, s)
		}
	}
